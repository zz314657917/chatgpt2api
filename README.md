...
推送 `v*` 标签后，GitHub Actions 会通过 GoReleaser 发布 GitHub Release、校验和、二进制压缩包和多架构 Docker 镜像。服务器默认拉取 `ghcr.io/zyphrzero/chatgpt2api:latest`，不在生产机现场安装依赖和编译。
...
release 构建也可以在设置页的“版本更新”卡片中检查 GitHub Release，并触发在线更新。在线更新会下载对应平台的 Release 压缩包，替换当前 `chatgpt2api` 二进制和 `web_dist` 前端产物，然后提示重启服务；Docker 容器依赖 `restart: unless-stopped` 拉起同一个容器。重新创建容器时仍以 Docker 镜像为准。
