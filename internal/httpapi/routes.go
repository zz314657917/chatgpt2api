...
func (a *App) handleImageTasks(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/api/image-tasks/generations" && r.Method == http.MethodPost {
		body, _ := readJSONMap(r)
		task, err := a.tasks.SubmitGeneration(r.Context(), identity, util.Clean(body["client_task_id"]), util.Clean(body["prompt"]), firstNonEmpty(util.Clean(body["model"]), util.ImageModelAuto), util.Clean(body["size"]), a.resolveImageBaseURL(r), util.ToInt(body["n"], 1), body["messages"])
		if err != nil {
			util.WriteError(w, http.StatusBadRequest, err.Error())
			return
		}
	...
	if r.URL.Path == "/api/image-tasks/edits" && r.Method == http.MethodPost {
		...
		task, err := a.tasks.SubmitEdit(r.Context(), identity, util.Clean(body["client_task_id"]), util.Clean(body["prompt"]), firstNonEmpty(util.Clean(body["model"]), util.ImageModelAuto), util.Clean(body["size"]), a.resolveImageBaseURL(r), images, util.ToInt(body["n"], 1), body["messages"])
		if err != nil {
			util.WriteError(w, http.StatusBadRequest, err.Error())
			return
		}
