...
var exactErrorTranslations = map[string]string{
	...
	"sub2api binding is incomplete":                                                          "Sub2API 绑定信息不完整",
	"sub2api binding store is unavailable":                                                   "Sub2API 绑定存储不可用",
	"sub2api launch is not configured":                                                       "Sub2API 启动登录未配置",
	"sub2api launch redeem is not configured":                                                "Sub2API 启动兑换未配置",
	"sub2api launch redeem payload is invalid":                                               "Sub2API 启动兑换响应格式无效",
	"sub2api launch redeem payload missing api_key.key":                                      "Sub2API 启动兑换响应缺少 api_key.key",
	"sub2api launch redeem payload missing gateway_base_url":                                 "Sub2API 启动兑换响应缺少 gateway_base_url",
	"sub2api launch redeem payload missing user.id":                                          "Sub2API 启动兑换响应缺少 user.id",
	"sub2api login did not return access_token":                                              "Sub2API 登录没有返回 access_token",
	"sub2api login payload is invalid":                                                       "Sub2API 登录响应格式无效",
	"sub2api server requires email+password or api_key":                                      "Sub2API 服务器需要邮箱密码或 api_key",
	"sub2api session was not created":                                                        "Sub2API 本地会话创建失败",
...
func localizePrefixError(text string) (string, bool) {
	prefixes := []struct {
		...
		{"sub2api launch redeem failed: ", "Sub2API 启动兑换失败："},
		{"sub2api login failed: ", "Sub2API 登录失败："},
		{"sub2api request failed: ", "Sub2API 请求失败："},
