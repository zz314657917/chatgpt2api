...
2. 如果继续推进 Sub2API 集成，补一份更清晰的 launch/redeem、leaf network login、图片任务、对象存储、`/image` 与 `/canvas` 之间的专题知识。
...
- 仅看 README 和旧任务快照，容易漏掉 per-user retention、continued edit、leaf network login、image workspace policy hardening，以及 `/canvas` 自研节点画布这些新默认约束。
