...
export const CANVAS_NODE_HELP: SmartCanvasNodeHelp[] = [
  {
    ...
    downstream: "API生成、AI 提示词、循环、组。",
    controls: ["文本框用于写主提示词。", "@图片 会把图片引用放进提示词节点，供下游读取。"],
    reminders: ["直接生图时连到 API生成。", "想先优化提示词时连到 AI 提示词。"],
  },
  {
    id: "image",
    title: "图片",
    ...
    downstream: "AI 提示词、API生成、循环、组。",
    controls: ["点击图片可预览。", "右下角拖拽可调整节点大小。"],
    reminders: ["连到 API生成 会作为图生图输入。", "连到 AI 提示词 会让模型先看图再产出提示词。"],
  },
  {
    id: "llm",
    title: "AI 提示词",
    ...
    downstream: "API生成、循环、Output。",
    controls: ["补充处理要求用于约束提词方式。", "模型选择决定使用哪个文本/多模态模型。", "生成提示词按钮会创建文本任务。"],
    reminders: ["AI 提示词看过的图片不会自动传给 API生成，界面会提示你点击连线补齐图片。"],
  },
  {
    id: "loop",
    title: "循环",
    summary: "按规则重复提交下游 API生成。",
    upstream: "提示词、图片、AI 提示词、组、Output。",
    downstream: "必须连接 API生成，再由 API生成 连接 Output。",
    controls: ["重复：同一组输入跑 N 轮。", "逐图：上游每张图片单独跑一轮。", "停止按钮可请求停止后续轮次。"],
    reminders: ["循环节点自己不生成图片，点击下游 API生成 才会运行。", "逐图模式的轮数由图片数量决定。"],
  },
  {
    id: "group",
    title: "组",
    ...
    downstream: "API生成、AI 提示词、循环、Result。",
  ...
  },
  {
    ...
    controls: ["补充提示词会追加到上游提示词后。", "模型、比例、可见性和张数控制本次提交参数。", "API生成按钮提交任务。"],
  ...
  },
  {
    ...
    upstream: "API生成、AI 提示词、循环、Image、组、Output。",
    downstream: "API生成、AI 提示词、循环、Image、组、Output。",
    controls: ["生成成功后显示图片或文本。", "图片可点击预览，也可继续连到其他节点。"],
    reminders: ["Output 不提交任务，只展示或传递结果。", "API生成 连接 Output 后结果会自动写入。"],
  },
];
...
export const CANVAS_FLOW_TEMPLATES: SmartCanvasFlowTemplate[] = [
  ...
  },
  {
    ...
    chain: "Prompt + Image -> API生成 -> Output",
  ...
  },
  {
    ...
    chain: "Prompt + Image -> AI提示词 -> API生成 -> Output",
  ...
  },
  {
    ...
    chain: "Prompt/Image -> 循环 -> API生成 -> Output",
  ...
  },
  {
    id: "loop-images",
    title: "逐图批量处理",
    summary: "多张上游图片逐张提交到 API生成。",
    chain: "多张 Image + Prompt -> 循环(逐图) -> API生成 -> Output",
  ...
  },
  {
    ...
    chain: "Prompt + Image -> 组 -> API生成 -> Output",
