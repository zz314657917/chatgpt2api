...
export function localizeErrorMessage(message: string): string {
    ...
    if (/\bsize\s+must\s+be\s+auto\s+or\s+width\s*x\s*height\b/.test(normalized)) {
        return "当前模型不支持比例尺寸，请改为“自动”尺寸，或使用“宽度x高度”（如 1024x1024）后重试。";
    }
...
request.interceptors.request.use(async (config) => {
    ...
    const skipStoredAuthorization = Boolean((nextConfig as RequestConfig).skipStoredAuthorization);
    const sessionToken = skipStoredAuthorization ? "" : await getStoredSessionToken();
